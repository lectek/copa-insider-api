  ...
  async function salvarEdicao() {
    ...
    if (file) {
      ...
      if (contentType && !ALLOWED_UPLOAD_IMAGE_TYPES.has(contentType)) {
        toast("Formato invalido. Use JPG, PNG ou WEBP.", true);
        return;
      }
      if (Number(file.size || 0) > MAX_UPLOAD_IMAGE_BYTES) {
        toast("Imagem acima de 20MB. Reduza o arquivo e tente novamente.", true);
        return;
      }

      const fd = new FormData();
      fd.append("file", file);
      const endpoint = `/api/admin/produtos/${encodeURIComponent(id)}/imagem`;
      const createRequest = () => withCsrf({
        method: "POST",
        body: fd,
        credentials: "same-origin"
      });
      let up = await fetch(endpoint, createRequest());
      if (!up.ok && isRetryableUploadStatus(up.status)) {
        await new Promise((resolve) => setTimeout(resolve, 450));
        up = await fetch(endpoint, createRequest());
      }
      if (!up.ok) {
        try {
          const msg = await responseToUploadError(up, `Falha no upload (HTTP ${up.status})`);
          const lower = String(msg).toLowerCase();
          if (/401|403|unauthorized|forbidden|full authentication/i.test(lower)) {
            toast("Sessao expirada. Faca login novamente para enviar a imagem.", true);
            ...
            toast("Dados salvos, mas o upload falhou no servidor. Use imagem ate 20MB.", true);
          } else {
            toast(msg || "Dados salvos, mas falhou ao enviar a imagem.", true);
          }
        } catch {
